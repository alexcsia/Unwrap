...
export const fetchListeningHistory = async (user: any): Promise<any> => {
    ...
    try {
        ...
        if (error.response?.status === 401 || error.message.includes("Bad or expired token")) {
        console.log(`Access token expired for user: ${user.id}. Attempting to refresh token...`);
        ...
        console.error(`Error fetching listening history for user ${user.id}:`, error);
        throw error;
    }
}
